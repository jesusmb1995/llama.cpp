...
std::optional<std::set<std::string>> parse_tensor_list_from_future(load_input_t & load_input) {
    ...
    llama_future_file_buffer_ro tensor_file(future_input.tensor_list_file.c_str(), future_input.context);
    llama_file_buffer_ro& file_buffer = tensor_file.get();
    const uint8_t* data = file_buffer.data();
    size_t size = file_buffer.size();
