...
gguf_file_load::gguf_file_load(struct ggml_context ** ctx, load_input_t load_input) :
    ...
    if (std::holds_alternative<fname_load_input>(load_input)) {
        ...
        std::unique_ptr<llama_file_buffer_ro> file_buffer = future_file->get();
        meta.reset(gguf_init_from_buffer(file_buffer->streambuf.get(), params));
        if (!meta) {
...
SplitsTensorLoad::SplitsTensorLoad(struct ggml_context * ctx, struct llama_model_loader & ml,
                                   gguf_file_load & base_split, std::set<std::string> tensor_list) :
...
void SplitsTensorLoad::add_split(SplitLoad splitLoad) {
...
void SplitsTensorLoad::_load_split(struct llama_model_loader & ml, uint16_t idx) {
...
void SplitsTensorLoad::_process_split(const struct ggml_context * ctx, struct llama_model_loader & ml, uint16_t idx) {
...
uint16_t SplitsTensorLoad::load_tensor_metadata(struct llama_model_loader & ml, const char * tensor_name,
                                              ggml_tensor ** out_tensor_metadata) {
    printf("SplitsTensorLoad::load_tensor_metadata loading tensor %s (tensor_meta=%p, delayed_loaded=%zu, delayed_files.size=%zu)\n",
...
uint16_t SplitsTensorLoad::get_split_idx_for_tensor(const char * tensor_name) const {
    return _get_tensor_info_iterator(tensor_name)->second.split_idx;
}

std::size_t SplitsTensorLoad::get_split_data_size(uint16_t split_idx) const {
    return _get_split_info_iterator(split_idx)->second.data_size;
}

void SplitsTensorLoad::print_currently_known_tensors() const {
    printf("Current incremental loaded tensors:\n");
    for (const auto & it : tensor_info) {
        printf("Tensor '%s' in split %d (loaded: %s)\n",
               it.first.c_str(), it.second.split_idx,
               it.second.is_loaded ? "yes" : "no");
    }
}

bool SplitsTensorLoad::all_tensors_are_loaded(uint16_t split_idx) const {
    auto it = _get_split_info_iterator(split_idx);
    const SplitInfo & split = it->second;
    printf("Loaded tensor count for split %d: %u/%u\n", split_idx, split.loaded_tensor_count, split.total_tensor_count);
    return split.all_tensors_loaded();
}

void SplitsTensorLoad::release_split(struct llama_model_loader &ml, uint16_t split_idx) {
    // Let destructor do release of memory
    ml.files[split_idx] = nullptr;
}

std::map<std::string, SplitsTensorLoad::TensorInfo>::const_iterator SplitsTensorLoad::_get_tensor_info_iterator(const char * tensor_name) const {
...
std::map<uint16_t, SplitsTensorLoad::SplitInfo>::const_iterator SplitsTensorLoad::_get_split_info_iterator(uint16_t split_idx) const {
...
bool SplitsTensorLoad::SplitInfo::all_tensors_loaded() const {
    return loaded_tensor_count >= total_tensor_count;
}

bool SplitsTensorLoad::tensor_ignored(const std::optional<SplitsTensorLoad> & splits_tensor_load, const char* tensor_name) {
    return !splits_tensor_load.has_value() || (splits_tensor_load.has_value() &&
