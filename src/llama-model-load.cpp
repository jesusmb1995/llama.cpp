
#include "llama-model-load.h"

#include <cstdint>
#include <memory>
#include <variant>

#include "llama-model-loader.h"

gguf_file_load::gguf_file_load(struct ggml_context ** ctx, load_input_t load_input) :
    params({
        /*.no_alloc = */ true,
        /*.ctx      = */ ctx,
    }) {
    using namespace load_input_variant;
    if (std::holds_alternative<fname_load_input>(load_input)) {
        const auto & file_input = std::get<fname_load_input>(load_input);
        meta.reset(gguf_init_from_file(file_input.fname.c_str(), params));
        if (!meta) {
            throw std::runtime_error(format("%s: failed to load model from %s", __func__, file_input.fname.c_str()));
        }
        file = std::make_unique<llama_file_disk>(file_input.fname.c_str(), "ro");
    } else if (std::holds_alternative<buffer_future_load_input>(load_input)) {
        const auto & future_input = std::get<buffer_future_load_input>(load_input);
        LLAMA_LOG_DEBUG("Loading model from buffer for %s\n", future_input.promise_key.c_str());
        auto future_file =
            std::make_unique<llama_future_file_buffer_ro>(future_input.promise_key, future_input.context);
        meta.reset(gguf_init_from_buffer(future_file->get().data(), future_file->get().size(), params));
        if (!meta) {
            throw std::runtime_error(format("%s: failed to load model from buffer", __func__));
        }
        file = std::move(future_file);
    } else {
        const auto & buffer_input = std::get<buffer_load_input>(load_input);
        meta.reset(gguf_init_from_buffer(buffer_input.data, buffer_input.size, params));
        if (!meta) {
            throw std::runtime_error(format("%s: failed to load model from buffer", __func__));
        }
        file = std::make_unique<llama_file_buffer_ro>(buffer_input.data, buffer_input.size);
    }
}

gguf_file_load SplitLoad::load_split_gguf(struct ggml_context ** ctx, const char * fname_split,
                                          load_input_t & load_input, std::vector<std::string> & splits) {
    using namespace load_input_variant;
    if (std::holds_alternative<fname_load_input>(load_input)) {
        return gguf_file_load(ctx, fname_load_input{ fname_split, splits });
    }
    if(std::holds_alternative<buffer_future_load_input>(load_input)) {
        auto future_input = std::get<buffer_future_load_input>(load_input);
        return gguf_file_load(ctx, buffer_future_load_input{ fname_split, future_input.context, splits });
    }
    return gguf_file_load(ctx, load_input);
}

SplitLoad::SplitLoad(load_input_t load_input, load_input_variant::fname_load_input base_split, uint16_t idx,
                     std::string kv_split_no) :
    load_input(load_input),
    base_split(base_split),
    idx(idx),
    kv_split_no(std::move(kv_split_no)) {}

SplitsTensorLoad::SplitsTensorLoad(struct ggml_context* ctx, struct llama_model_loader & ml, gguf_file_load& base_split) {
    ml.process_loaded_gguf(ctx, base_split, 0);
    processSplit(ctx, ml, 0);
}

struct ggml_context * SplitLoad::load(llama_model_loader & ml) {
    if (loaded) {
        return ml.contexts[idx].get();
    }

    struct ggml_context * ctx         = ml.contexts.back().get();

    const char * fname_split = base_split.splits[idx].c_str();
    printf("Fully Loading file %s\n", fname_split);

    gguf_file_load     split_gguf = gguf_file_load(load_split_gguf(&ctx, fname_split, load_input, base_split.splits));
    gguf_context_ptr & split_meta = split_gguf.meta;

    if(idx > 0)
    {
        const int kid = gguf_find_key(split_meta.get(), kv_split_no.c_str());
        if (kid < 0) {
            throw std::runtime_error(format("missing key %s in GGUF split %s", kv_split_no.c_str(), fname_split));
        }
        int idx_gguf = gguf_get_val_u16(split_meta.get(), kid);
        if (idx_gguf != idx) {
            throw std::runtime_error(
                format("invalid split file idx: %d (file: %s), expected %d", idx_gguf, fname_split, idx));
        }
    }

    // Check that this split's idx matches the expected position in ml.files
    if (!ml.files.empty() && idx != ml.files.size()) {
        throw std::runtime_error(
            format("invalid split file loading order: got idx %d but expected %zu based on ml.files size", idx,
                   ml.files.size()));
    }

    ml.process_loaded_gguf(ctx, split_gguf, idx);

    loaded = true;
    return ctx;
}

void SplitsTensorLoad::addSplit(SplitLoad splitLoad) {
    // +1 because first split is expected to have been already loded (not delayed)
    split_to_loaded_tensor_count[delayed_files.size() + 1] = 0;
    delayed_files.emplace_back(std::move(splitLoad));
}

void SplitsTensorLoad::loadSplit(struct llama_model_loader & ml, uint16_t idx) {
    // -1 because first split is expected to have been already loded (not delayed and not present in delayed_files)
    const struct ggml_context * ctx = delayed_files[idx - 1].load(ml);
    processSplit(ctx, ml, idx);
}

void SplitsTensorLoad::processSplit(const struct ggml_context * ctx, struct llama_model_loader & ml, uint16_t idx) {
    int split_tensor_count = 0;
    for (ggml_tensor * cur = ggml_get_first_tensor(ctx); cur; cur = ggml_get_next_tensor(ctx, cur)) {
        std::string tensor_name = std::string(cur->name);
        split_tensor_count++;
        tensor_to_split[tensor_name] = idx;
        auto it                      = ml.weights_map.find(tensor_name);
        if (it == ml.weights_map.end()) {
            throw std::runtime_error(format("tensor '%s' not found in weights_map", tensor_name.c_str()));
        }
        split_to_size_data[idx] += ggml_nbytes(it->second.tensor);
    }
    split_to_tensor_count[idx] = split_tensor_count;
}

uint16_t SplitsTensorLoad::loadTensorMetadata(struct llama_model_loader & ml, const char * tensor_name,
                                              ggml_tensor ** out_tensor_metadata) {
    printf("SplitsTensorLoad::loadTensorMetadata loading tensor %s (tensor_meta=%p, delayed_loaded=%zu, delayed_files.size=%zu)\n",
           tensor_name, (void*)*out_tensor_metadata, delayed_loaded, delayed_files.size());
    while (!(*out_tensor_metadata) && delayed_loaded < delayed_files.size()) {
        printf("Loading file %i\n", (int) delayed_loaded);  // TODO correct formatting type
        // +1 because first split is expected to have been already loded (not delayed)
        loadSplit(ml, delayed_loaded + 1);
        *out_tensor_metadata = ml.get_tensor_meta(tensor_name);
        delayed_loaded++;
        if (*out_tensor_metadata) {
            printf("Tensor found in file %i\n", (int) delayed_loaded);
        }
    }
    uint16_t split_idx = getSplitIdxForTensor(tensor_name);
    if(!tensor_to_loaded[tensor_name]) {
        split_to_loaded_tensor_count[split_idx]++;
        tensor_to_loaded[tensor_name] = true;
    }
    return split_idx;
}

uint16_t SplitsTensorLoad::getSplitIdxForTensor(const char * tensor_name) const {
    auto it = tensor_to_split.find(tensor_name);
    if (it == tensor_to_split.end()) {
        throw std::runtime_error(format("tensor '%s' not found in tensor_to_split map", tensor_name));
    }
    return it->second;
}

std::size_t SplitsTensorLoad::getSplitDataSize(uint16_t split_idx) const {
    auto it = split_to_size_data.find(split_idx);
    if (it == split_to_size_data.end()) {
        throw std::runtime_error(format("split index %d not found in split_to_size_data map", split_idx));
    }
    return it->second;
}

void SplitsTensorLoad::printCurrentlyKnownTensors() const {
    printf("Current incremental loaded tensors:\n");
    for (const auto & it : tensor_to_split) {
        printf("Tensor '%s' in split %d\n", it.first.c_str(), it.second);
    }
}

bool SplitsTensorLoad::allTensorsAreLoaded(uint16_t split_idx) const {
    auto loaded_it = split_to_loaded_tensor_count.find(split_idx);
    auto total_it = split_to_tensor_count.find(split_idx);
    printf("Loaded tensor count for split %d: %d/%d\n", split_idx, loaded_it->second, total_it->second);
    if (loaded_it == split_to_loaded_tensor_count.end() || total_it == split_to_tensor_count.end()) {
        throw std::runtime_error(format("split index %d not found in tensor count maps", split_idx));
    }
    return loaded_it->second >= total_it->second;
}

void SplitsTensorLoad::releaseSplit(struct llama_model_loader &ml, uint16_t split_idx) {
    // TODO: Let destructor do release
    ml.files[split_idx]->release();
    ml.files[split_idx] = nullptr;
}
