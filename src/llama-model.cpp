...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            ggml_context * ctx = split_idx.has_value() ? ml.incremental_splits_tensor_load->get_model_ctx_for_split_buft(
            ...
            if (split_idx.has_value() && ml.incremental_splits_tensor_load->all_tensors_are_loaded(*split_idx)) {
                // Upload right now.
                if (!create_backend_buffers(*split_idx, ml.incremental_splits_tensor_load->ctx_split_map, ml, use_mmap_buffer, use_mlock, n_gpu_layers)) {
...
/// @brief Create backend buffers for a split of the model. Removes the split from the map.
...
bool llama_model::create_backend_buffers(std::size_t size_data, const std::map<ggml_backend_buffer_type_t, ggml_context *>& ctx_map, llama_model_loader& ml, const bool use_mmap_buffer, const bool use_mlock, const int32_t n_gpu_layers, bool do_print_backend_buffers_info) {
