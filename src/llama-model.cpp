...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    auto ctx_for_split_buft = [&](ggml_backend_buffer_type_t buft, int split) -> ggml_context * {
        ...
        if (it == ctx_split_map.end()) {
            printf("creating context for split %d\n", split);
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            if(!t_meta && (flags & TENSOR_NOT_REQUIRED)) {
                return nullptr; // TODO: rethink TENSOR_NOT_REQUIRED
            }
            if(ml.splits_tensor_load.has_value()) {
                split_idx = ml.splits_tensor_load->loadTensorMetadata(ml,  tn.str().c_str(), &t_meta);
                printf("Split idx for tensor %s: %d\n", tn.str().c_str(), *split_idx);
            }
            if (!t_meta) {
            ...
            ggml_context * ctx = split_idx.has_value() ? ctx_for_split_buft(buft, *split_idx) : ctx_for_buft(buft);
            ...
            if (split_idx.has_value() && ml.splits_tensor_load->allTensorsAreLoaded(*split_idx)) {
                // Upload right now.
                ...
                SplitsTensorLoad::releaseSplit(ml, *split_idx);
            }
...
bool llama_model::create_backend_buffers(
    ...
    std::size_t split_data_size = ml.splits_tensor_load->getSplitDataSize(idx);
    printf("Creating backend buffers for split %d with size %zu\n", idx, split_data_size);
    const bool creation_success = create_backend_buffers(split_data_size, ctx_map, ml, use_mmap_buffer, use_mlock, n_gpu_layers);
