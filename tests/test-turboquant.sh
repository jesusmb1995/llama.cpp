...
# self-spawns child processes with GGML_VK_TBQ_COPY_SG_SIZE set to {0, 4, 8,
# 16, 32, 64} and checks that GPU quantization matches the CPU reference
# byte-for-byte (up to known ties) for every TBQ/PQ variant. SG values the
# device does not expose are rejected host-side with a warning and labelled
# SKIP-<reason> in the per-case table (they run at the device default, which
# duplicates the sg=0 row and doesn't provide independent coverage). Requires
# a Vulkan-capable backend; harmlessly skips otherwise.
