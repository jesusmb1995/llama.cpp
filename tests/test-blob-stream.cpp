...
int main(int argc, char * argv[]) {
    ...
    for (int test = 0; test < 100; ++test) {
        ...
        for (int consecutive_read = 0; consecutive_read < consecutive_reads_gen; consecutive_read++) {
            ...
            assert(blobs_bytes_read == (std::size_t) direct_istream.gcount());
        ...
        if (test % 20 == 0) {
            std::cout << "Test " << test << ": seek to " << seek_pos << ", read " << total_read
                      << " bytes (consecutive_reads=" << consecutive_reads_gen << ") - PASSED" << std::endl;
        }
    }
