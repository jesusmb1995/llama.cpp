...
def run_job(job: Job, extra_args: list[str], output_dir: Path) -> Optional[str]:
    ...
    with _print_lock:
        print(f"  DONE:  {label}  =>  {accuracy}  {progress_str}")
    return csv_path
        ...
        with open(path) as f:
            ...
            for row in reader:
                try:
                    ...
                    print(f"  WARN: bad CSV row in {path}: {e}")
    return results
...
def main():
    ...
    if args.rerun_missing:
        ...
        if output_dir is None:
            print(f"No existing '{base_dir}' dir found, creating fresh one.")
            output_dir = base_dir
        else:
            print(f"Rerun-missing: reusing {output_dir}")
    else:
    ...
    if not available_models:
        print("ERROR: No model files found. Expected:")
        for m in models:
            print(f"  {m.path}")
        sys.exit(1)
    ...
    if missing:
        print("WARNING: Skipping missing models:")
        for m in missing:
            print(f"  {m.path}")
    ...
    if args.rerun_missing:
        ...
        for job in all_jobs:
            ...
            if csv_p.exists() and csv_p.stat().st_size > 50:
                print(f"  SKIP (exists): {job.model.label} K={job.k_type} V={job.v_type} ({job.tag})")
                if job.tag == "main":
    ...
    print("")
    print("=" * 60)
    print(" KV Cache RULER Benchmark")
    print("=" * 60)
    print(f"  Config:    {preset.name}")
    print(f"  Samples:   {preset.num_samples}")
    print(f"  Contexts:  {preset.ctx_lengths}")
    print(f"  GPUs:      {gpu_ids}")
    print(f"  Models:    {len(available_models)}")
    for m in available_models:
        print(f"    - {m.label}: {m.path}")
    print(f"  Quants:    {len(preset.quant_configs)}")
    for k, v in preset.quant_configs:
        print(f"    - K={k} V={v} ({bpw_label(k, v)})")
    n_skipped = len(skipped_csvs_main) + len(skipped_csvs_v2) if args.rerun_missing else 0
    print(f"  Jobs:      {len(all_jobs)} to run" +
          (f" ({n_skipped} skipped with existing results)" if n_skipped else ""))
    ...
    if args.dry_run:
        ...
        for j in all_jobs:
            print(f"  [GPU{j.gpu_id}] {j.model.label} K={j.k_type} V={j.v_type} "
                  f"tasks='{j.tasks}' ctx='{j.ctx_lengths}' n={j.num_samples} ({j.tag})")
        return
    ...
    if all_jobs:
        ...
        print(f"\nRunning {len(all_jobs)} jobs across {len(gpu_ids)} GPU(s)...\n")
    ...
    with open(report_path, "w") as f:
        f.write(f"KV Cache RULER Benchmark Report\n")
        f.write(f"Config: {preset.name}\n")
        f.write(f"Date: {timestamp}\n")
        f.write(f"Command: python3 {' '.join(sys.argv)}\n")
        f.write(f"Models: {', '.join(m.label for m in available_models)}\n\n")
        for t in all_tables:
            f.write(t)
            f.write("\n\n")
    print(f"Report: {report_path}")

    print(f"\nJob logs/CSVs: {output_dir / 'jobs'}/")
    print("Done.")
