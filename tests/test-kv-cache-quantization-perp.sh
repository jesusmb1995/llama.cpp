...
#   huge   — 8 chunks, n_ctx 448,512,1024,2048,4096  (full sweep, 5 context sizes)
#
...
csv_init() {
    if [ -n "$CSV_FILE" ]; then
        echo "gpu_device,model,config,cache_k,cache_v,mixed,norm_correction,bpw_avg,n_chunks,n_ctx_sweep,ppl_mean,ppl_stdev,time_mean_s,time_stdev_s,ppl_vs_f16_pct,ppl_vs_f16_pct_stdev,time_vs_f16_x,time_vs_f16_x_stdev" > "$CSV_FILE"
    fi
}
...
csv_append() {
    ...
    local row="\"$GPU_DEVICE\",\"$MODEL_NAME\",\"$config_name\",\"$k_type\",\"$v_type\",\"$is_mixed\",\"$NC_FLAG\",$bpw_avg,$n_chunks,\"$n_ctx_sweep\",$ppl_mean,$ppl_sd,$time_mean,$time_sd,$ppl_vs_f16,$ppl_vs_f16_sd,$time_vs_f16,$time_vs_f16_sd"
    CSV_ROWS+=("$row")
    if [ -n "$CSV_FILE" ]; then
        echo "$row" >> "$CSV_FILE"
    fi
...
run_perplexity_once() {
    ...
    echo "      -m $MODEL_PATH \\" >&2
    echo "      -f $DATASET_FILE \\" >&2
    echo "      --cache-type-k $k_type --cache-type-v $v_type \\" >&2
    echo "      -fa on -n $n_ctx --chunks $n_chunks" >&2
...
run_perplexity_sweep() {
    ...
    if [ "$n_runs" -eq 1 ]; then
        ...
        local mean_binary_sd
        mean_binary_sd=$(printf '%s\n' "${ppl_sds[@]}" | awk '{s+=$1*$1; n++} END {printf "%.4f", sqrt(s/n)}')
        # combined stdev: root-sum-of-squares of across-ctx spread and mean binary stdev
        ppl_stdev=$(echo "$sweep_sd $mean_binary_sd" | awk '{printf "%.4f", sqrt($1*$1 + $2*$2)}')
...
for cfg in "${CONFIGS[@]}"; do
    ...
    for pair in "${RUN_CONFIGS[@]}"; do
        ...
        ppl_sd_results["$pair"]=$(echo "$result" | awk '{print $2}')
        time_results["$pair"]=$(echo "$result" | awk '{print $3}')
        time_sd_results["$pair"]=$(echo "$result" | awk '{print $4}')
        echo "  K=$k_type V=$v_type PPL = $(fmt_pm "${ppl_results[$pair]}" "${ppl_sd_results[$pair]}")  ($(fmt_pm "${time_results[$pair]}" "${time_sd_results[$pair]}")s)"
        echo ""
        ...
        if is_tq_type "$k_type" || is_tq_type "$v_type"; then
            ...
            ppl_sd_results_a["$pair"]=$(echo "$result" | awk '{print $2}')
            time_results_a["$pair"]=$(echo "$result" | awk '{print $3}')
            time_sd_results_a["$pair"]=$(echo "$result" | awk '{print $4}')
            echo "  K=$k_type V=$v_type (α) PPL = $(fmt_pm "${ppl_results_a[$pair]}" "${ppl_sd_results_a[$pair]}")  ($(fmt_pm "${time_results_a[$pair]}" "${time_sd_results_a[$pair]}")s)"
            echo ""
        fi
    done
    ...
    if [ ${#REF_CONFIGS[@]} -gt 0 ]; then
        ...
        for pair in "${REF_CONFIGS[@]}"; do
            ...
            echo "--- Running perplexity with K=ref-$k_type V=ref-$v_type [$cfg] (ref impl) ---"
            ...
            ppl_sd_results["$ref_key"]=$(echo "$result" | awk '{print $2}')
            time_results["$ref_key"]=$(echo "$result" | awk '{print $3}')
            time_sd_results["$ref_key"]=$(echo "$result" | awk '{print $4}')
            echo "  K=ref-$k_type V=ref-$v_type PPL = $(fmt_pm "${ppl_results[$ref_key]}" "${ppl_sd_results[$ref_key]}")  ($(fmt_pm "${time_results[$ref_key]}" "${time_sd_results[$ref_key]}")s)"
            echo ""
        done
        unset SWEEP_BIN
    fi
    ...
    echo "=========================================="
    if [ "$HAS_STDEV" -eq 1 ]; then
        printf "  %-10s %-10s %-6s %16s %18s %16s\n" "K type" "V type" "BPW" "PPL (mean±sd)" "vs f16" "Time"
    else
        printf "  %-10s %-10s %-6s %10s %12s %10s\n" "K type" "V type" "BPW" "PPL" "vs f16" "Time"
    fi
    printf "  %-10s %-10s %-6s %16s %18s %16s\n" "------" "------" "---" "---" "------" "----"
    ...
    if [ ${#REF_CONFIGS[@]} -gt 0 ]; then
        ...
        for pair in "${REF_CONFIGS[@]}"; do
            ...
            reg_disp=$(fmt_regression "$ppl" "$ppl_sd" "$baseline_ppl")
            slow_disp=$(fmt_slowdown "$elapsed" "$elapsed_sd" "$baseline_time" "$baseline_time_sd")
            printf "  %-10s %-10s %-6s %16s %18s %12s (%s)\n" "$k_disp" "$v_disp" "-" "$ppl_disp" "$reg_disp" "$time_disp" "$slow_disp"
        done
    fi
    ...
    if [ -n "$CSV_FILE" ]; then
        ...
        for pair in "${RUN_CONFIGS[@]}"; do
            ...
            csv_append "$k_type" "$v_type" "$is_m" "$N_CHUNKS_CFG" "$CTX_SWEEP_STR" "$ppl" "$ppl_sd" "$elapsed" "$elapsed_sd" "$ppl_vs_f16" "$ppl_vs_f16_sd" "$time_vs_f16" "$time_vs_f16_sd" "$cfg"
        done
        ...
        if [ ${#REF_CONFIGS[@]} -gt 0 ]; then
            ...
            for pair in "${REF_CONFIGS[@]}"; do
                ...
                time_vs_f16=$(echo "$elapsed $baseline_time" | awk '{if ($2 > 0) printf "%.2f", $1/$2; else print ""}')
                time_vs_f16_sd=$(csv_ratio_sd "$elapsed" "$elapsed_sd" "$baseline_time" "$baseline_time_sd")
                csv_append "$k_disp" "$v_disp" "$is_m" "$N_CHUNKS_CFG" "$CTX_SWEEP_STR" "$ppl" "$ppl_sd" "$elapsed" "$elapsed_sd" "$ppl_vs_f16" "$ppl_vs_f16_sd" "$time_vs_f16" "$time_vs_f16_sd" "$cfg"
            done
        fi
    fi
